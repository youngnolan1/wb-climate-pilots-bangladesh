#-------------------------------------------------------------------------------
# Script: regressions_salesperworker.R
...
revenue_reg_data <- reg_master[complete.cases(reg_master$sales_per_worker), ]

#----------------------------- 2. TEMPERATURE ----------------------------------

# Run reg
temp_model <- lm(sales_per_worker ~ avg_temp + avg_temp:firm_age + avg_temp:size + avg_temp:sampling_region + 
...
# Run reg
tempvolatility_model <- lm(sales_per_worker ~ temp_volatility + temp_volatility:firm_age + temp_volatility:size + temp_volatility:sampling_region + 
...
# Run reg
hotdays_model <- lm(sales_per_worker ~ total_hotdays + total_hotdays:firm_age + total_hotdays:size + total_hotdays:sampling_region + 
...
# Run reg
spei_model <- lm(sales_per_worker ~ avg_spei + avg_spei:firm_age + avg_spei:size + avg_spei:sampling_region + 
