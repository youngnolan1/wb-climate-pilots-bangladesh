...
# Check which businesses have telephone contact info
tele <- business %>% 
  filter(!is.na(Tele)) %>% 
  filter(Tele != 0) %>% 
  filter(Tele != ".")
...
business <- business %>%
  ...
  select(Establishment_cd, zilla_id, zilla_name_comite_eng, BSIC, Tele) %>% 
  rename(ZillaName = zilla_name_comite_eng)

# Check which manufacturng businesses have telephone contact info
tele <- business %>% 
  filter(!is.na(Tele)) %>% 
  filter(Tele != 0) %>% 
  filter(Tele != ".")
...
smi19_firm_count_sans_dhaka <- merged %>% 
  filter(ZillaName != "DHAKA") %>% 
  group_by(geometry) %>% 
  summarize(HeatVar = sum(Establishment_cd))
